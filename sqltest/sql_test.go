...
func (m *goSpannerDB) RunTest(t *testing.T, fn func(params)) {
	if !m.Running() {
		t.Logf("skipping test; no Spanner running on localhost:3306")
		return
	}
	...
	dbName := "gosqltest"
	db, err := sql.Open("spanner", fmt.Sprintf("%s:%s@/%s", user, pass, dbName))
	if err != nil {
		t.Fatalf("error connecting: %v", err)
	}

	params := params{goSpanner, t, db}
...
func TestBlobs_GoSpanner(t *testing.T) { goSpanner.RunTest(t, testBlobs) }
...
func TestManyQueryRow_GoSpanner(t *testing.T) { goSpanner.RunTest(t, testManyQueryRow) }
...
func TestTxQuery_GoSpanner(t *testing.T) { goSpanner.RunTest(t, testTxQuery) }
...
func TestPreparedStmt_GoSpanner(t *testing.T) { goSpanner.RunTest(t, testPreparedStmt) }
